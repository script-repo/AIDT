...
// running aidt can self-update (install.ps1 renames the locked exe).
...
printf '[Service]\nEnvironment="OLLAMA_API_KEY=%s"\n' '%s' > /etc/systemd/system/ollama.service.d/aidt-key.conf
...
// Kept for compatibility with existing persisted state migrations/tests.
