...
// parseBatchEndpoint extracts an endpoint from an "AIDT_ENDPOINT <json>" log
// line (the tag prefix, if any, is ignored).
func parseBatchEndpoint(line string) (batchEndpoint, bool) {
	const marker = "AIDT_ENDPOINT "
...
// parseVMRecord extracts a deploy-time "AIDT_VM {name,image,ip}" line emitted
