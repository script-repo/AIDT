...
// agentPathBootstrap makes an agent behave the same however it is started.
//
// agentOpenCmd exports the Olla wiring inline — OPENCODE_CONFIG, GROK_HOME,
// CODEX_HOME, and the goose/claude env files — so an agent launched by AIDT
// reaches the pool. None of it was persisted, so the same binary started from
// any other shell fell back to its own default configuration and talked to its
// vendor's endpoint instead. That is invisible until you notice the traffic
// never arrives at the gateway.
//
// The settings live in one generated file that every deploy rewrites, with a
// single sourcing line in ~/.bashrc, so refreshing them never has to edit
// ~/.bashrc again.
const agentPathBootstrap = `mkdir -p "$HOME/.config/aidt"
cat > "$HOME/.config/aidt/agent-env.sh" <<'AIDT_AGENT_ENV'
# Generated by AIDT. Points the agent CLIs at the Olla gateway for every shell
# on this host, not only the ones AIDT launches. Regenerated on each deploy.

# Prepend once, so repeated sourcing cannot grow PATH without bound.
case ":$PATH:" in
  *":$HOME/.opencode/bin:"*) ;;
  *) export PATH="$HOME/.local/bin:$HOME/.npm-global/bin:$HOME/.opencode/bin:$HOME/.grok/bin:$HOME/.codex/bin:$PATH" ;;
esac

# Each guarded on its own file, so only the agents actually installed here apply.
[ -r "$HOME/.config/aidt/opencode.json" ] && export OPENCODE_CONFIG="$HOME/.config/aidt/opencode.json"
[ -d "$HOME/.config/aidt/grok" ]          && export GROK_HOME="$HOME/.config/aidt/grok"
[ -d "$HOME/.config/aidt/codex" ]         && export CODEX_HOME="$HOME/.config/aidt/codex"
[ -r "$HOME/.config/aidt/codex.env" ]      && { set -a; . "$HOME/.config/aidt/codex.env"; set +a; }
[ -r "$HOME/.config/aidt/claude-code.env" ] && { set -a; . "$HOME/.config/aidt/claude-code.env"; set +a; }
[ -r "$HOME/.config/aidt/goose.env" ]      && { set -a; . "$HOME/.config/aidt/goose.env"; set +a; export GOOSE_PROVIDER=olla; }
:
AIDT_AGENT_ENV
chmod 600 "$HOME/.config/aidt/agent-env.sh"

# Replace any previous block, including the PATH-only one earlier versions
# wrote, so an upgrade does not leave two of them behind.
if [ -f "$HOME/.bashrc" ]; then
  sed -i -e '/^# >>> AIDT agent env >>>$/,/^# <<< AIDT agent env <<<$/d' -e '/^# AIDT agent tools$/,+1d' "$HOME/.bashrc"
fi
cat >> "$HOME/.bashrc" <<'AIDT_AGENT_RC'
# >>> AIDT agent env >>>
[ -r "$HOME/.config/aidt/agent-env.sh" ] && . "$HOME/.config/aidt/agent-env.sh"
# <<< AIDT agent env <<<
AIDT_AGENT_RC
echo "[deploy] agent CLIs point at Olla in every shell (~/.config/aidt/agent-env.sh)"
`
