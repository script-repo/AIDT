...
//  1. $AIDT_PYTHON (explicit override)
...
func vmScriptPath() string {
	if p := os.Getenv("AIDT_VM_SCRIPT"); p != "" {
		return p
	}
...
func localOllaScriptPath() string {
	if p := os.Getenv("AIDT_OLLA_SCRIPT"); p != "" {
		return p
	}
...
func RunLocalOllaInstall(ch chan<- ProcEvent) {
	...
	if _, err := os.Stat(script); err != nil {
		ch <- ProcEvent{Line: "could not find install-olla.sh (" + err.Error() + "); set AIDT_OLLA_SCRIPT", Done: true, Code: 1}
		return
	}
	var cmd *exec.Cmd
	if os.Geteuid() == 0 {
