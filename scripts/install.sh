...
# Override the version with AIDT_VERSION=v1.2.3 and the location with
...
VERSION="${AIDT_VERSION:-}"
if [ -z "$VERSION" ]; then
  info "resolving latest release"
  VERSION="$(fetch "https://api.github.com/repos/$REPO/releases/latest" \
    | grep -o '"tag_name"[[:space:]]*:[[:space:]]*"[^"]*"' \
    | head -n1 | sed 's/.*"\([^"]*\)"$/\1/')"
  [ -n "$VERSION" ] || fail "could not determine latest release; set AIDT_VERSION"
fi
VER_NO_V="${VERSION#v}"
...
# running aidt (Update local machine) is replaced cleanly on Unix.
stage="$tmp/stage"
mkdir -p "$stage"
...
BIN_DIR="${AIDT_BIN_DIR:-$HOME/.local/bin}"
...
# Set AIDT_SKIP_DEPS=1 to skip this section entirely.
have() { command -v "$1" >/dev/null 2>&1; }

if [ "${AIDT_SKIP_DEPS:-0}" = "1" ]; then
