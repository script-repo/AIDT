<#
.SYNOPSIS
  One-line installer for the AI-Deployment-Toolkit TUI (Windows).
...
.NOTES
  Override with env vars before running:
    $env:AIDT_VERSION     = 'v1.2.3'   # pin a release (default: latest)
...
$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("aidt-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
  ...
  # aidt.exe may be the currently running process (Windows locks it and
...
# everything. Set $env:AIDT_SKIP_DEPS = '1' to skip this section.
function Have($name) { [bool](Get-Command $name -ErrorAction SilentlyContinue) }

if ($env:AIDT_SKIP_DEPS -eq '1') {
  Info 'skipping dependency setup (AIDT_SKIP_DEPS=1)'
} else {
