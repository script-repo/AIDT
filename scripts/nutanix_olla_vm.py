...
The guest password is supplied per run (no default is baked in):
  ...
  AIDT_VM_PASSWORD the cloud-init password for the 'rocky' user, or pass
...
DEFAULT_IMAGE_NAME = os.environ.get("AIDT_IMAGE_NAME", "")
DEFAULT_CLUSTER_NAME = os.environ.get("AIDT_CLUSTER_NAME", "")
DEFAULT_SUBNET_NAME = os.environ.get("AIDT_SUBNET_NAME", "")
...
STATE_DIR = Path.home() / ".ai-deployment-toolkit"
...
def build_vm_body(
    *,
    ...
    return {
        ...
        "name": name,
        "description": "Provisioned by AI-Deployment-Toolkit nutanix_olla_vm.py",
...
def provision_vm(pc: PrismClient, args: argparse.Namespace, vm_name: str) -> dict:
    ...
    print("AIDT_VM " + json.dumps({"name": vm_name, "image": args.image_name, "ip": ip}))
    return {"vm_ext_id": vm_ext_id, "ip": ip, "name": vm_name}


def install_tui_on_guest(ssh: Ssh) -> bool:
    """Install the aidt CLI on the guest as the unprivileged VM user, so an
    ...
    log("installing aidt on the gateway")
    if installer.exists():
    ...
    if rc != 0:
        log(f"warning: aidt install returned {rc}; the gateway itself is unaffected")
        if err:
            log(f"aidt install stderr: {err.strip()[:500]}")
        if out:
            log(f"aidt install stdout tail: {out.strip()[-500:]}")
        return False
    # Verify the binary landed.
    vrc, vout, _ = ssh.run(
        'export PATH="$HOME/.local/bin:$PATH"; command -v aidt || ls -la "$HOME/.ai-deployment-toolkit/aidt" 2>/dev/null',
        stream=False,
    )
    if vrc == 0 and vout.strip():
        log(f"aidt installed: {vout.strip().splitlines()[-1]}")
        return True
    log("warning: aidt install finished but binary not found on PATH")
    return False
...
def finish_pattern_a(ip: str, args: argparse.Namespace, vm_name: str, vm_ext_id: str | None = None) -> None:
    ...
    if tui_installed:
        log(f"manage it from the gateway: ssh {args.vm_user}@{ip} then run aidt")
...
def finish_pattern_b(ip: str, args: argparse.Namespace, vm_name: str, vm_ext_id: str | None = None) -> None:
    ...
    if getattr(args, "no_register", False):
        registered = False
        print("AIDT_ENDPOINT " + json.dumps(endpoint))
...
def add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--ssh-pubkey", default=os.environ.get("AIDT_SSH_PUBKEY", ""),
                   help="Public key (authorized_keys line) to authorize for --vm-user at first "
                        "boot, so later SSH needs no password (or env AIDT_SSH_PUBKEY)")
    p.add_argument("--ip-prefix", default=os.environ.get("AIDT_IP_PREFIX", ""),
                   help="Prefer a learned IP with this subnet prefix (default: take the first learned IP; "
                        "or set AIDT_IP_PREFIX)")
...
def main() -> int:
    ...
    pa.add_argument("--no-install-tui", action="store_true",
                    help="Skip installing aidt on the gateway (it is installed by "
    ...
    pb.add_argument("--no-register", action="store_true",
                    help="Provision + install only; skip Olla registration and print "
                         "'AIDT_ENDPOINT <json>' (used for batched parallel deploys)")
    ...
    # is baked in, so require it explicitly (flag or AIDT_VM_PASSWORD).
    if args.command in ("pattern-a", "pattern-b", "pattern-custom", "install-a", "install-b", "register-endpoints") and not getattr(args, "vm_password", ""):
        fatal("VM password required: pass --vm-password or set AIDT_VM_PASSWORD "
    ...
    if args.command in ("pattern-a", "pattern-b", "pattern-custom"):
        ...
        if prov_missing:
            fatal("missing required placement option(s): " + ", ".join(prov_missing) +
                  ...
                  "AIDT_IMAGE_NAME / AIDT_CLUSTER_NAME / AIDT_SUBNET_NAME)")
